<template>
    <div class="bg-white p-6 rounded-lg shadow-md">
      <h2 class="text-xl font-semibold mb-4">Heure de travail</h2>
      <div class="mb-4">
      ...
      <p v-if="!chartData">Pas de données valide</p>
    </div>
</template>
        ...
        try {
          ...
          switch (selectedPeriod.value) {
            ...
            case 'year':
              ...
              labels = ['Jan', 'Fev', 'Mar', 'Apr', 'Mai', 'Jun', 'Jul', 'Aou', 'Sep', 'Oct', 'Nov', 'Dec'];
              break;
          }
  
          const response = await api.getWorkingTimes(props.userId, start.toISOString(), end.toISOString());
          ...
          workingTimes.forEach(wt => {
            ...
            switch (selectedPeriod.value) {
              case 'week':
                index = startDate.getDay() - 1;
                if (index === -1) index = 6; // Dimanche
                break;
              case 'month':
