  ...
  export default {
    ...
    methods: {
      async saveWorkingTime() {
        try {
          if (this.workingTimeId) {
            ...
            await axios.put(`localhost:4000/api/working_times/${this.userId}/${this.workingTimeId}`, {
              start_time: this.workingTime.start,
              end_time: this.workingTime.end
            });
          } else {
            
            await axios.post(`localhost:4000/api/working_times/${this.userId}`, {
              start_time: this.workingTime.start,
              end_time: this.workingTime.end
            });
          }
      ...
      async deleteWorkingTime() {
        try {
          await axios.delete(`localhost:4000/api/working_times/${this.userId}/${this.workingTimeId}`);
        } catch (error) {
